...
export default class MergeResults {
    private testDataResults: Map<string, string> = new Map<string, string>();
    ...
    public async aggregateResults() {
        ...
        await fs.readdirSync(this.resultPath).forEach(async file => {
            const data = await fs.readFileSync(`${this.resultPath}/${file}`, 'utf-8');
            const convertMap = new Map<string, string>(Object.entries(JSON.parse(data)));
    ...
    public async convertMilliToHhMmSsMs(milliseconds: number): Promise<string> {
        await console.log("The duration is " + milliseconds);
    ...
    public async getPassCount(): Promise<number> {
        let count = 0;
    ...
    public async getPagesCount(): Promise<Map<string, number>> {
        const pageCountMap: Map<string, number> = new Map<string, number>();
        for (const key of this.testDataResults.keys()) {
    ...
    public async getPassCountPerPage(page: string): Promise<number> {
        let passcount = 0;
        for (const key of this.testDataResults.keys()) {
    ...
    public async getTotalExecutionTime(): Promise<string> {
        let totalTime = 0;
        for (const key of this.testDataResults.keys()) {
            totalTime = totalTime + Number(this.testDataResults.get(key).split("^")[1]);
        }
        return await this.convertMilliToHhMmSsMs(totalTime);
    }

    public async getTotalExecutionTimePerPage(page: string): Promise<string> {
        let totalTime = 0;
        for (const key of this.testDataResults.keys()) {
    ...
    public async createEmailBodyHtmlFile() {
        ...
        await fs.writeFileSync(`${this.resultPath}/emailBodyReport.html`, content);
    }

    public async write(content: string) {
        await fs.writeFileSync(`${this.resultPath}/emailBodyReport.html`, content, { flag: 'a+' });
    }

    public async insertAggregateTableData(suite: string, brand: string) {
        ...
        content = `${content}\n<td align="center" style="padding:0;background:#33FFF6;">\n${await this.getPassCount()}\n</td>`;
        content = `${content}\n<td align="center" style="padding:0;background:#FF5E33;">\n${this.testDataResults.size - await this.getPassCount()}\n</td>`;
        content = `${content}\n<td align="center" style="padding:0;">\n${await this.getTotalExecutionTime()}\n</td>`;
        content = `${content}\n</tr>\n</table>\n`;
        await this.write(content);
    }

    public async insertIndividualTableData() {
        let content = `\n<br><br>\n`;
        ...
        const pageCountMap = await this.getPagesCount();
        let colorCount = 2;
        for (const keyPage of pageCountMap.keys()) {
            ...
            for (const key of this.testDataResults.keys()) {
                if (key.split("^")[0] === keyPage) {
                    ...
                    const executionTime = await this.convertMilliToHhMmSsMs(Number(this.testDataResults.get(key).split("^")[1]));
                    await console.log("The Execution time is " + executionTime);
                    content = `${content}\n<td align="center">\n${executionTime}\n</td>`;
                    content = `${content}\n</tr>`;
                    rowCount++;
                }
    ...
    public async createEmailableReport(envProps) {
        await this.aggregateResults();
        await this.createEmailBodyHtmlFile();
        await this.insertAggregateTableData("BVT", "FTD");
        await this.insertIndividualTableData();

        await this.createRepresentiveReport();
    }

    public async createRepresentiveReport() {
        const totalResults = this.testDataResults.size;
        const passedCount = await this.getPassCount();
        ...
        for (const page of pageCountMap.keys()) {
            ...
            const totalExectionTimeForPage = await this.getTotalExecutionTimePerPage(page);
            content = `${content}\n<span class="metal-text">${totalExectionTimeForPage}</span>`;
            content = `${content}\n${page}`;
            content = `${content}\n<div class="progress-container">`;
            const passCountForPage = await this.getPassCountPerPage(page);
            const totalTestCasesPerPage = (await this.getPagesCount()).get(page);
        ...
        content = `${content}\n</body></html>`;
        await fs.writeFileSync(`${this.resultPath}/representable-report.html`, content);
